...
struct gatts_service_inst* gatts_services[SERVICE_ID_COUNT] = {
    [SERVICE_ID_IMU] = &gatts_service_imu,
};
struct connection_t curr_con = {
    .conn_id = 0,
    .gatts_if = ESP_GATT_IF_NONE,
    .connected = false
};

#define BLE_SERVICE_UUID_SIZE SERVICE_ID_COUNT*ESP_UUID_LEN_128 // 2 services -> 2 128bit service UUIDs
static uint8_t ble_service_uuid128[BLE_SERVICE_UUID_SIZE] = {
	UUID_SERVICE_IMU_128,
};
...
static esp_ble_adv_data_t ble_adv_data = {
	...
	.service_data_len = BLE_ADV_SERVICE_DATA_LEN,
	.p_service_data = BLE_ADV_SERVICE_DATA,
...
void gaps_init() {
	...
	// merge all the service
	uint16_t i = 0, j = 0, c_sum = 0;
	for(uint8_t s = 0; s < SERVICE_ID_COUNT; s++){
		for(uint16_t c = 0; c < gatts_services[s]->chrc_count; c++){
			gatts_char[i] = gatts_services[s]->char_list[c];
			gatts_char[i].service_pos = s;
			i++;
		}
		for(uint16_t d = 0; d < gatts_services[s]->desp_count; d++){
			gatts_descr[j] = gatts_services[s]->descr_list[d];
			gatts_descr[j].char_pos += c_sum;
			j++;
		}
		c_sum += gatts_services[s]->chrc_count;
	}

	char device_name[BLE_DEVICE_NAME_LEN] = BLE_DEVICE_NAME;
	// snprintf(device_name, BLE_DEVICE_NAME_LEN, BLE_DEVICE_NAME, *gatts_char[GATTS_BUTTON_NUMBER_CHAR_POS].char_val->attr_value); // copy configured button number into the device name
...
static uint16_t ble_add_char_pos;
static uint16_t ble_add_descr_pos;
 ...
 * 
 * TODO: Can be improved by binary search
 */
static void gatts_read_value_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
	...
	for (uint16_t pos=0;pos<GATTS_CHAR_NUM;pos++) {
		if (gatts_char[pos].char_handle==param->read.handle) {
			...
			if (gatts_char[pos].char_val!=NULL) {
				...
				for (uint16_t valpos=0;valpos<gatts_char[pos].char_val->attr_len&&valpos<gatts_char[pos].char_val->attr_max_len;valpos++) {
	...
	for (uint16_t pos=0;pos<GATTS_DESCR_NUM;pos++) {
	...
	for (uint16_t pos=0;pos<GATTS_CHAR_NUM;pos++) {
		if (gatts_char[pos].char_handle==param->write.handle) {
			...
			if (gatts_char[pos].char_val!=NULL) {
				...
				if(gatts_char[pos].val_write != NULL){
					gatts_char[pos].val_write(param->write.len, param->write.value, param->write.offset);
				}
				...
				ESP_LOGI(GATTS_TAG, "gatts_write_value_handler %.*s", gatts_char[pos].char_val->attr_len, (char*)gatts_char[pos].char_val->attr_value);
	...
	for (uint16_t pos=0;pos<GATTS_DESCR_NUM;pos++) {
		if (gatts_descr[pos].descr_handle==param->write.handle) {
			...
			if (gatts_descr[pos].descr_val!=NULL) {
				...
				if(gatts_descr[pos].val_write != NULL){
					gatts_descr[pos].val_write(param->write.len, param->write.value, param->write.offset);
				}
				...
				ESP_LOGI(GATTS_TAG, "gatts_write_value_handler: wrote: %.*s", gatts_descr[pos].descr_val->attr_len, (char*)gatts_descr[pos].descr_val->attr_value);
...
static void gatts_add_char() {
	...
	for (uint16_t pos=0;pos<GATTS_CHAR_NUM;pos++) {
		if (gatts_char[pos].service_pos==ble_add_service_pos && gatts_char[pos].char_handle==0) {
			ESP_LOGI(GATTS_TAG, "gatts_add_char: adding char pos %d to service pos %d (service handle %d)", pos, ble_add_service_pos, gatts_services[ble_add_service_pos]->service_handle);
			ble_add_char_pos=pos;
			esp_ble_gatts_add_char(gatts_services[ble_add_service_pos]->service_handle, &gatts_char[pos].char_uuid,
...
static void gatts_add_descr() {
	...
	for (uint16_t pos=0;pos<GATTS_DESCR_NUM;pos++) {
		if (gatts_descr[pos].descr_handle==0 && gatts_descr[pos].char_pos==ble_add_char_pos) {
			ESP_LOGI(GATTS_TAG, 
                     "gatts_add_descr: adding descr pos %d to char pos %d (handle %d) on service %d (handle %d)", 
                     pos, ble_add_char_pos, gatts_char[ble_add_char_pos].char_handle, ble_add_service_pos, 
                     gatts_services[ble_add_service_pos]->service_handle);
			ble_add_descr_pos=pos;
			esp_ble_gatts_add_char_descr(gatts_services[ble_add_service_pos]->service_handle, &gatts_descr[pos].descr_uuid,
...
static void gatts_check_add_char(esp_bt_uuid_t char_uuid, uint16_t attr_handle) {
	...
	if (attr_handle != 0) {
		if (char_uuid.len == ESP_UUID_LEN_16) {
			...
			ESP_LOGI(GATTS_TAG, "Char UUID32: %lx", char_uuid.uuid.uuid32);
...
static void gatts_check_add_descr(esp_bt_uuid_t descr_uuid, uint16_t attr_handle) {
	...
	if (attr_handle != 0) {
		if (descr_uuid.len == ESP_UUID_LEN_16) {
			...
			ESP_LOGI(GATTS_TAG, "Char UUID32: %lx", descr_uuid.uuid.uuid32);
...
void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
	...
	case ESP_GATTS_REG_EVT:
		...
		if (param->reg.status == ESP_GATT_OK) {
			gatts_services[param->reg.app_id]->gatts_if = gatts_if;
		} else {
		...
		gatts_services[param->reg.app_id]->service_id.is_primary = true;
		gatts_services[param->reg.app_id]->service_id.id.inst_id = 0x00;
		gatts_services[param->reg.app_id]->service_id.id.uuid.len = ESP_UUID_LEN_128;
		for (uint8_t pos=0;pos<ESP_UUID_LEN_128;pos++) {
			// copy correct part of ble_service_uuid128 byte by byte into the service struct
			gatts_services[param->reg.app_id]->service_id.id.uuid.uuid.uuid128[pos]=ble_service_uuid128[pos+16*param->reg.app_id];
		...
		ESP_LOGI(GATTS_TAG, "ble_service_uuid128[0] %d, gatts_service[param].uuid128[0] %d, gatts_service[ble_pos].uuid128[0] %d", 
                            ble_service_uuid128[0], gatts_services[param->reg.app_id]->service_id.id.uuid.uuid.uuid128[0], 
                            gatts_services[ble_add_service_pos]->service_id.id.uuid.uuid.uuid128[0]);

		esp_ble_gatts_create_service(gatts_if, &gatts_services[param->reg.app_id]->service_id, gatts_services[param->reg.app_id]->num_handles);
		break;
	case ESP_GATTS_READ_EVT: {
		ESP_LOGI(GATTS_TAG, "GATT_READ_EVT, conn_id %d, trans_id %ld, handle %d", param->read.conn_id, param->read.trans_id, param->read.handle);
	...
	case ESP_GATTS_WRITE_EVT: {
		...
		ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, value len %d, value %08lx", param->write.len, *(uint32_t *)param->write.value);
	...
	case ESP_GATTS_CREATE_EVT:
		...
		gatts_services[ble_add_service_pos]->service_handle = param->create.service_handle;
		ESP_LOGI(GATTS_TAG, "param->create.service_handle %d, gatts_service[param->reg.app_id].service_handle %d, gatts_service[ble_add_service_pos].service_handle %d\n", param->create.service_handle, gatts_services[param->reg.app_id]->service_handle, gatts_services[ble_add_service_pos]->service_handle);

		esp_ble_gatts_start_service(gatts_services[ble_add_service_pos]->service_handle);
		gatts_add_char();
		break;
	...
	case ESP_GATTS_CONNECT_EVT: {
		...
		gatts_services[ble_add_service_pos]->conn_id = param->connect.conn_id;
		//start send the update connection parameters to the peer device.
		esp_ble_gap_update_conn_params(&conn_params);
		curr_con.conn_id = param->connect.conn_id;
		curr_con.connected = true;
		curr_con.gatts_if = gatts_if;
		// // update LED pattern:
		// status_connected = true;
		break;
	}
	case ESP_GATTS_DISCONNECT_EVT:
		curr_con.connected = false;
		curr_con.conn_id = 0;
		curr_con.gatts_if = ESP_GATT_IF_NONE;
...
void gatt_notify(uint16_t handle, uint16_t value_len, uint8_t* value, bool need_confirm){
	if(curr_con.connected){			
		int error = esp_ble_gatts_send_indicate(
										curr_con.gatts_if, 
										curr_con.conn_id, 
										handle,
										value_len, 
										value, 
										need_confirm);
		if(error) ESP_LOGI("error", "%d", error); 
	}
}